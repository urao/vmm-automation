...
if [ -n "${CLEANUP:-}" ] || [ ! -z "${FORCE_CLEANUP:-}" ]; then
  ...
  echo CLEANING: DEMO VMM topology
  ansible-playbook -i inventory/ ${extra_args:-} playbooks/vmm_cleanup.yml
else
  # Run playbook creation
  echo RUNNING: DEMO VMM topology playbooks
  ansible-playbook -i inventory/ ${extra_args:-} playbooks/provision_vmm.yml ${TAGS:-}
  ansible-playbook -i inventory/${VMM_TOPO_TYPE} ${extra_args:-} playbooks/deploy_demo_vmm.yml ${TAGS:-}
fi
